...
int Search::node(int current_depth, int max_depth, bool &search_flag, Color maximizing_player) {
    ...
    if (current_depth == max_depth) {        
        ...
        if (state == GameState::DRAW_BY_75_MOVE_RULE || 
            ...
            state == GameState::DRAW_BY_THREEFOLD_REPETITION) score = 0;
        else if (state == GameState::CHECKMATE && current_turn == maximizing_player) score = -1000000;
        else if (state == GameState::CHECKMATE) score = 1000000;
        else score = Evaluation::evaluate_board_for(_game.get_board(), maximizing_player);
        _game.next_turn();
        return score;
    }
