...
void go(std::vector<std::string>& args, Engine& engine) {
    ...
    engine.start_search(
        go_options.count(GO_OPTIONS::DEPTH) ? std::optional<int>(std::stoi(go_options.at(GO_OPTIONS::DEPTH)))
                                            : std::nullopt,
        go_options.count(GO_OPTIONS::MOVE_TIME) ? std::optional<int>{std::stoi(go_options.at(GO_OPTIONS::MOVE_TIME))}
                                                : std::nullopt,
        go_options.count(GO_OPTIONS::W_TIME) ? std::optional<int>(std::stoi(go_options.at(GO_OPTIONS::W_TIME)))
                                             : std::nullopt,
        go_options.count(GO_OPTIONS::B_TIME) ? std::optional<int>(std::stoi(go_options.at(GO_OPTIONS::B_TIME)))
                                             : std::nullopt,
        go_options.count(GO_OPTIONS::W_INC) ? std::optional<int>(std::stoi(go_options.at(GO_OPTIONS::W_INC)))
                                            : std::nullopt,
        go_options.count(GO_OPTIONS::B_INC) ? std::optional<int>(std::stoi(go_options.at(GO_OPTIONS::B_INC)))
                                            : std::nullopt,
